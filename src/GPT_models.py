...
# ──────────────────────────────────────────────────────────────────────────────
def get_gpu_power():
    """Return current GPU power draw in Watts via nvidia-smi."""
    cmd = "nvidia-smi --query-gpu=power.draw --format=csv,noheader,nounits"
    try:
        out = subprocess.check_output(cmd, shell=True).decode().splitlines()[0]
        return float(out)
    except:
        return None
...
# TRAIN STEP
# ──────────────────────────────────────────────────────────────────────────────
def hf_train_step(model, opt, input_ids, attention_mask, labels):
    opt.zero_grad()
    with autocast():
...
# BENCHMARK FUNCTION
# ──────────────────────────────────────────────────────────────────────────────
def gpt_from_config(L, E, H, vocab_size, batch_size, seq_length,
                    repeats, tracker, lr=1e-3):
    """Instantiate GPT-2 with config (L, E, H) and measure training energy."""
    ...
    # Measure
    name = f"L{L}_E{E}_H{H}_bs{batch_size}_sl{seq_length}"
...
def main():
    ...
    out_csv   = f"gpt2_customcfg_grig{today}_{hour}H.csv"
    emiss_csv = f"gpt2_customcfg_grig_emiss_{today}_{hour}H.csv"
    tracker   = EmissionsTracker(output_file=emiss_csv, log_level="ERROR")

    vocab_size      = 50257
    repeats         = 3
    seq_length      = 256
    batch_size_list = np.arange(32, 32*11, 32)
    batch_size_list =[int(i) for i in batch_size_list]
    layers          = [2, 4, 6, 12]
    d_models        = np.arange(64*2, 64*2*11, 64*2)
    d_models =[int(i) for i in d_models]
    H               = 8
    ...
    for bs in batch_size_list:
        for L in layers:
            for E in d_models:
                seq_len_list = np.arange(64*2, 64*2 * 11, 64 * 2)
                for seq_length in seq_len_list:
                    if int(E) % H != 0:
                        continue
                    cfg_str = f"bs={bs},L={L},E={E},H={H}"
                    try:
                        print("Running", cfg_str)
                        stats = gpt_from_config(
                            L, E, H, vocab_size,
                            bs, seq_length,
                            repeats, tracker
                        )
